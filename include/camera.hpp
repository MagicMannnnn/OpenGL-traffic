...
public:
    ...
    void ProcessKeyboard(Camera_Movement direction, float deltaTime) {
        ...
        // Calculate direction vectors ignoring vertical component of the front vector
        glm::vec3 forward = glm::normalize(glm::vec3(Front.x, 0.f, Front.z)); // Keep movement on the X-Z plane
        glm::vec3 right = glm::normalize(glm::vec3(Right.x, 0.f, Right.z)); // Right direction on X-Z plane

        if (direction == FORWARD) {
            Position += forward * velocity;  // Move forward on the X-Z plane
        }
        if (direction == BACKWARD) {
            Position -= forward * velocity;  // Move backward on the X-Z plane
        }
        if (direction == LEFT) {
            Position -= right * velocity;  // Move left on the X-Z plane
        }
        if (direction == RIGHT) {
            Position += right * velocity;  // Move right on the X-Z plane
        }
        if (direction == UP) {
            Position += WorldUp * velocity;  // Move up along the Y-axis
        }
        if (direction == DOWN) {
            Position -= WorldUp * velocity;  // Move down along the Y-axis
        }

        // Clamp the camera's vertical position to a maximum and minimum height
